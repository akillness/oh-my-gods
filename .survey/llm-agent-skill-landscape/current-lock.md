...
- Primary lane: resume the measured `ohmg` loop with one scored mutation at a time from clean `main`.
...
- Smallest bounded action for this run: advance the loop onto a fresh post-merge branch, repair the lock state after merged PR `#14`, and ignore the known generated byproducts before the next scored `ohmg` experiment

## Status

- Current state: PR `#14` merged into `main` on 2026-04-12, this follow-on branch is limited to lock-state repair plus ignore coverage for generated loop byproducts, and the next substantive lane is `ohmg` optimization
- Blocker: `ohmg` still fails to consistently anchor Gemini-or-Antigravity framing plus Serena Memory, and the next run should score one focused mutation instead of reopening structure work
- Next owner: `nanoclaw_engine`
- Stage: `improvement`
- PR-readiness note: no new PR should be opened until the next `ohmg` mutation or another equally bounded follow-up slice is ready
