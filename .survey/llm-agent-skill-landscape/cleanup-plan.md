# Cleanup Plan: `api-design` packaging lane

## Goal

Advance the recurring improvement loop after PR `#63` closed the
`code-refactoring` lane by standardizing `api-design`. Keep the work focused on
...
- Keep this run focused on `api-design` only.
- Do not reopen already-merged unrelated lanes.
- Keep the support package decision explicit:
  - `assets`: no
  - `scripts`: no
  - `references`: yes, add focused contract-boundary and review guidance
  - `evals`: yes, add trigger, route-out, and compatibility-design checks
- Do not start a `skill-autoresearch` mutation loop until the packaged lane
  still shows measured failures.

## Planned checks

1. Re-read `api-design` for generic or catch-all wording.
...
- Survey refresh now shows PR `#63` merged cleanly, so the loop should stop
  treating `code-refactoring` as active.
- `api-design` is the strongest remaining general-purpose gap because it is
  still workflow-critical and still lacks focused `references/` and `evals/`.
- This run should package `api-design`, open the PR path, and hand the next
