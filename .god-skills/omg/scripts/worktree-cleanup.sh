#!/usr/bin/env bash
# OMG Skill — Worktree Cleanup Script
...
if [[ -f "$STATE_FILE" ]] && command -v python3 >/dev/null 2>&1; then
  if $DRY_RUN; then
    echo -e "${YELLOW}[DRY-RUN]${NC} Would update OMG state: phase=cleanup, cleanup_completed=true"
  else
...
try:
    ...
    print("✓ OMG state updated: cleanup complete")
