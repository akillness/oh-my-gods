...
`superpowers` 스킬 정의를 Agent Skills와 함께 로드할 수 있습니다. `omg`를 오케스트레이션 레이어로 사용하고 `superpowers` 스킬을 컴포저블 프리미티브로 활용하세요:

```bash
# superpowers 스킬 컨텍스트를 Agent Skills와 함께 로드
omg "superpowers 스킬 계약을 사용하여 REST API 구현"
```
...
`bkit-claude-code`와 Agent Skills는 상호 보완적입니다 — `bmad`로 고수준 단계 게이팅을 하고 `bkit`으로 PDCA 품질 루프를 적용하세요:

```bash
# 예시: bmad 구조 + bkit PDCA 품질 루프
omg "사용자 인증 구현"  # bmad가 단계 관리, bkit이 품질 검증
```
...
| omg — 통합 오케스트레이션 스킬 | [../../.agent-skills/omg/SKILL.md](../../.agent-skills/omg/SKILL.md) |
| bmad — 단계 기반 개발 | [../bmad/README.md](../bmad/README.md) |
